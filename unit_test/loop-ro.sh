...
run_test "$LOOP_SCRIPT archive $fpath_ro" "0" "$(escape_expected "Archived [.roh.git] to [/Users/dev/Project-@knev/readonlyhash.sh.git/Fotos [space]/1999.ro/_.roh.git.zip].*Removed [/Users/dev/Project-@knev/readonlyhash.sh.git/Fotos [space]/1999.ro/.roh.git].*Archived [.roh.git] to [/Users/dev/Project-@knev/readonlyhash.sh.git/2002.ro/_.roh.git.zip].*Removed [/Users/dev/Project-@knev/readonlyhash.sh.git/2002.ro/.roh.git]")"
...
run_test "$LOOP_SCRIPT verify $fpath_ro" "0" "$(escape_expected "On branch master.*nothing to commit, working tree clean.*Removed [/var/folders/.*/tmp.*].*On branch master.*nothing to commit, working tree clean.*Removed [/var/folders/.*/tmp.*]")"

run_test "$LOOP_SCRIPT extract $fpath_ro" "0" "ERROR" "true"
run_test "$LOOP_SCRIPT verify $fpath_ro" "0" "ERROR" "true"
run_test "$LOOP_SCRIPT verify $fpath_ro" "0" "$(escape_expected "Removed [/var/folders/.*/tmp.*].*Removed [/var/folders/.*/tmp.*]")" "true"
run_test "$LOOP_SCRIPT verify $fpath_ro" "0" "$(escape_expected "Done..*On branch master.*nothing to commit, working tree clean.*Done..*On branch master.*nothing to commit, working tree clean")"
