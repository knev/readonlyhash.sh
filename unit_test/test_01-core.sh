...
echo
echo "# --sha"
rm -rf "shatest"; mkdir -p "shatest/sub"
echo "alpha" > "shatest/a.txt"; echo "beta" > "shatest/sub/b.txt"
run_test "$FPATH_BIN write --sha 999 shatest" "1" "$(escape_expected "ERROR: --sha must be one of 256, 384 or 512 (got [999])")"
run_test "$FPATH_BIN write --sha 512 shatest" "0" "ERROR:" "true"
run_test "ls shatest/.roh.git/a.txt.sha512 shatest/.roh.git/sub/b.txt.sha512" "0" "a.txt.sha512"
run_test "wc -c < shatest/.roh.git/a.txt.sha512" "0" "129"
run_test "$FPATH_BIN verify --sha 512 shatest" "0" "ERROR:" "true"
# hashes of another algorithm are orphans: switch by deleting first
run_test "$FPATH_BIN verify --sha 384 shatest" "1" "$(escape_expected "a.txt.sha512] orphaned hash")"
run_test "$FPATH_BIN delete --sha 512 shatest" "0" "ERROR:" "true"
run_test "$FPATH_BIN write --sha 384 shatest" "0" "ERROR:" "true"
run_test "wc -c < shatest/.roh.git/a.txt.sha384" "0" "97"
run_test "$FPATH_BIN verify --sha 384 shatest" "0" "ERROR:" "true"
run_test "$FPATH_BIN delete --sha 384 shatest" "0" "ERROR:" "true"
run_test "$FPATH_BIN write --sha 512 shatest" "0" "ERROR:" "true"
echo "gamma" > "shatest/a.txt"
run_test "$FPATH_BIN verify --sha 512 shatest" "1" "$(escape_expected "ERROR: hash mismatch")"
run_test "$FPATH_BIN write index --sha 512 --db shatest.db --force shatest" "0" "ERROR:" "true"
run_test "sqlite3 shatest.db 'select length(hash) from hashes limit 1'" "0" "^128$"
rm -rf "shatest" "shatest.db"
